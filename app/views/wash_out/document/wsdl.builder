...
xml.definitions 'xmlns' => 'http://schemas.xmlsoap.org/wsdl/',
  ...
  xml.portType :name => "#{@name}_port" do
    @map.each do |operation, formats|
      xml.operation :name => operation do
        xml.input :message => "#{@namespace}:#{operation}"
        xml.output :message => "#{@namespace}:#{formats[:response_tag]}"
      end
    end
  end

  xml.binding :name => "#{@name}_binding", :type => "#{@namespace}:#{@name}_port" do
  ...
  xml.service :name => @service_name do
    xml.port :name => "#{@name}_port", :binding => "#{@namespace}:#{@name}_binding" do
